...
int sekv_append(int sockfd, char *key, int flags, int exptime, int bytes, char *value)
{
    ...
    memset(bufv,0,BUFSIZ);memset(sb,0,BUFSIZ);
    char *command="append ";
    strcpy(comm, command);
    // encrypt <key>
    unsigned char *p_enc, *p_mac;
    ...
    strncat(comm,p_mac,16);
    strcat(comm,p_enc);
    // the value length bytes+16, 16 is the length of MAC, 20 bytes of meta sha1, 4 bytes of version number,8 bytes of command name, 16 bytes of update number
    sprintf(buf," %d %d %d",flags,exptime,bytes+16+20+4+8+16);
    strcat(comm,buf);
    strcat(comm,"\r\n");
    printf("command:\n");
    BIO_dump_fp(stdout,comm,strlen(comm));
...
//    strncpy(it_new->key,p_mac,16);
    ...
    if(it){
//        assoc_delete(it_new->key,it_new->nkey, hv);
        printf("append command: find the same key!\n");
    }else{
        printf("append command: not find the same key!\n");
    }
//    assoc_insert(it_new, hv);
    ...
    free(p_mac);
    // encrypt <MACmeta,vn,value>
    p_enc = (unsigned char *)malloc(sizeof(unsigned char)*(strlen(value)+20+4+8+16));
    p_mac = (unsigned char *)malloc(sizeof(unsigned char)*16);
    // MACmeta is the sha1 message digest
    unsigned char *md_value;
    int md_len;
    ...
    sprintf(buf,"%4d",0);
    strncat(bufv,buf,4);
    sprintf(buf,"%8s","append");
    strncat(bufv,buf,8);
    mcounter++;
    sprintf(buf,"%16d",mcounter);
    strncat(bufv,buf,16);
    strcat(bufv,value);
    printf("bufv:%s\n",bufv);
    printf("strlen(bufv):%d\n",strlen(bufv));
